...
def gen(videostream):
    ...
    while True:
        ...
        print(leftmotor_count)
        print(rightmotor_count)
        
        
        
        '''
        if leftstate != leftmotor_count:
            leftstate = leftmotor_count
            doc_ref_drone = db.collection(u'robot1').document(u'control')
            doc_ref_drone.set({
            u'left_right': leftstate,
            u'up_down': 0
            })
        
        if rightstate != rightmotor_count:
            rightstate = rightmotor_count
            doc_ref_drone = db.collection(u'robot1').document(u'control')
            doc_ref_drone.set({
            u'left_right': rightstate,
            u'up_down': 0
            })
         '''   
